...
test('migration upgrades legacy data under foreign keys and preserves references',()=>{
  ...
  try {
    ...
    // schema.sql defines structure only (no seeded rows - see seed/dev-seed.sql),
    // so this test creates its own minimal tenant/company fixture rather than
    // relying on any pre-existing seed data.
    execFileSync('sqlite3',[db],{input:`PRAGMA foreign_keys=ON; INSERT INTO tenants(id,name) VALUES('legacy-tenant','Legacy Tenant'); INSERT INTO companies(id,tenant_id,sage_company_name) VALUES('legacy-company','legacy-tenant','Legacy Co'); INSERT INTO users(id,tenant_id,email,name) VALUES('legacy-user','legacy-tenant','legacy@example.com','Legacy'); CREATE TABLE user_refs(id TEXT PRIMARY KEY,user_id TEXT NOT NULL REFERENCES users(id)); INSERT INTO user_refs VALUES('ref','legacy-user'); INSERT INTO connector_jobs(id,tenant_id,company_id,request_id,action,payload,status) VALUES('legacy-job','legacy-tenant','legacy-company','legacy-request','customer.create','{}','processing');`});
...
// The accepted Beta 1 API exposes quote creation but not quote sync/read routes.
// Quote authorization is exercised through the company-scoped create job below.
test('authenticated reads remain company-scoped for customers, invoices, and products',async()=>{
  const {mf,env}=await database();
  try {
    const token=jwt('read-user');
    const boot=await json(await handleRequest(request('/auth/bootstrap','POST',token),env));
    const company=boot.body.companies[0].id;
    const org=boot.body.organization.id;
    await env.DB.batch([
      env.DB.prepare(`INSERT INTO customers(tenant_id,company_id,sage_id,name,balance) VALUES (?,?,?,'Read Customer',0)`).bind(org,company,'C-READ'),
      env.DB.prepare(`INSERT INTO invoices(tenant_id,company_id,sage_id,invoice_number,date,total,balance) VALUES (?,?,?,'INV-READ','2026-09-13',10,0)`).bind(org,company,'I-READ'),
      env.DB.prepare(`INSERT INTO products(tenant_id,company_id,sage_id,sku,name,description,price) VALUES (?,?,?,'SKU-READ','Read Product','Read Product',10)`).bind(org,company,'P-READ'),
    ]);
    for (const [path,key] of [['/api/customers','customers'],['/api/invoices','invoices'],['/api/products','all']] as const) {
      const listed=await json(await handleRequest(request(path,'GET',token,undefined,{'x-company-id':company}),env));
      assert.equal(listed.status,200,path);
      assert.equal(listed.body[key].length,1,path);
    }
  } finally {await mf.dispose()}
});
...
test('routes enforce tenant isolation, pairing lifecycle, connector scope, provisioning and job replay',async()=>{
  ...
  try {
    ...
    assert.equal((await json(await handleRequest(request('/api/customers'),env))).status,401);
    assert.equal((await json(await handleRequest(request('/api/customers','GET','not-a-jwt'),env))).status,401);
    ...
    assert.equal((await json(await handleRequest(request('/api/customers','GET',tokenA,undefined,{'x-company-id':'cmp_forged'}),env))).status,403);
    assert.equal((await json(await handleRequest(request(`/api/companies/${companyB}/connectors`,'GET',tokenA),env))).status,403);
    await env.DB.prepare(`UPDATE organization_memberships SET status='revoked' WHERE organization_id=? AND user_id=(SELECT id FROM users WHERE auth_subject='user-a')`).bind(bootA.body.organization.id).run();
    assert.equal((await json(await handleRequest(request('/api/customers','GET',tokenA,undefined,{'x-company-id':companyA}),env))).status,403);
    await env.DB.prepare(`UPDATE organization_memberships SET status='active' WHERE organization_id=? AND user_id=(SELECT id FROM users WHERE auth_subject='user-a')`).bind(bootA.body.organization.id).run();
    ...
    await env.DB.prepare(`UPDATE connector_credentials SET expires_at=datetime('now','-1 minute') WHERE connector_id=?`).bind(connectorId).run();
    assert.equal((await json(await handleRequest(connectorRequest('/connector/jobs','GET',connectorId,connectorToken),env))).status,401);
    await env.DB.prepare(`UPDATE connector_credentials SET expires_at=NULL WHERE connector_id=?`).bind(connectorId).run();
    ...
    // A connector credential cannot select a different company on sync routes.
    assert.equal((await json(await handleRequest(connectorRequest('/sync/customers','POST',exchangeB.body.connectorId,exchangeB.body.credential,{TenantId:exchange.body.organizationId,CompanyId:companyA,Customers:[]}),env))).status,409);
...
test('invoice sync ignores removed due date and derived status fields',async()=>{
  const {mf,env}=await database();
  try {
    ...
    // Removed connector fields may still arrive from an older connector during
    // rollout, but the Beta API must neither persist nor expose them.
    const sync=await json(await handleRequest(connectorRequest('/sync/invoices','POST',connectorId,credential,{TenantId:boot.body.organization.id,CompanyId:company,Invoices:[
      {Id:'INV-ZERO',InvoiceNumber:'1001',Date:'2026-01-01',DueDate:'2026-01-31',Total:500,Balance:0,Status:'Paid'},
      {Id:'INV-BALANCE',InvoiceNumber:'1002',Date:'2026-02-01',DueDate:'2026-03-03',Total:750.5,Balance:250.25,Status:'Unpaid'},
    ]}),env));
    assert.equal(sync.status,200); assert.equal(sync.body.synced,2);
    ...
    assert.equal(bySageId['INV-ZERO'].balance,0);
    assert.equal(bySageId['INV-BALANCE'].balance,250.25);
    assert.equal(bySageId['INV-BALANCE'].total,750.5);
    for (const invoice of list.body.invoices) {
      assert.ok(!('dueDate' in invoice));
      assert.ok(!('status' in invoice));
    }
  } finally {await mf.dispose()}
});
