...
async function requireSyncScope(request: Request, organizationId: string, companyId: string) {
  const body=await request.clone().json() as Record<string,unknown>;
  const requestedOrganization=body.TenantId ?? body.tenantId;
  const requestedCompany=body.CompanyId ?? body.companyId;
  if ((requestedOrganization !== undefined && requestedOrganization !== organizationId) ||
      (requestedCompany !== undefined && requestedCompany !== companyId)) {
    throw new HttpError(409,'Sync payload scope does not match connector','CONNECTOR_SCOPE_MISMATCH');
  }
}
...
async function routeRequest(request:Request,env:Env):Promise<Response>{
    ...
    if(path.startsWith('/connector/')||path.startsWith('/sync/')) {
      ...
      if(path==='/sync/customers'&&method==='POST') { await requireSyncScope(request,connector.organizationId,connector.companyId); return handleSyncCustomers(request,env); }
      if(path==='/sync/invoices'&&method==='POST') { await requireSyncScope(request,connector.organizationId,connector.companyId); return handleSyncInvoices(request,env); }
      if(path==='/sync/products'&&method==='POST') { await requireSyncScope(request,connector.organizationId,connector.companyId); return handleSyncProducts(request,env); }
