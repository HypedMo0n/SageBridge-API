...
export async function exchangePairing(request: Request, env: Env, ip: string) {
  ...
  const connectorMutation=existing
    ? env.DB.prepare(`UPDATE connectors SET display_name=?,machine_name=?,version=?,connector_version=?,status='active',revoked_at=NULL,last_seen_at=CURRENT_TIMESTAMP,paired_at=CURRENT_TIMESTAMP,provisioning_state='connector_connected',updated_at=CURRENT_TIMESTAMP WHERE id=? AND organization_id=? AND company_id=? AND installation_id=?`).bind(machineName,machineName,connectorVersion,connectorVersion,connector,row.organization_id,row.company_id,installation)
    : env.DB.prepare(`INSERT INTO connectors(id,organization_id,company_id,installation_id,display_name,machine_name,version,connector_version,last_seen_at,paired_at,provisioning_state) SELECT ?,organization_id,company_id,?,?,?,?,?,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,'connector_connected' FROM pairing_codes WHERE id=? AND claimed_connector_id=?`).bind(connector,installation,machineName,machineName,connectorVersion,connectorVersion,row.id,connector);
  const results=await env.DB.batch([
    env.DB.prepare(`UPDATE pairing_codes SET status='consumed',consumed_at=CURRENT_TIMESTAMP,attempts=attempts+1,claimed_connector_id=? WHERE id=? AND status='active' AND expires_at>CURRENT_TIMESTAMP AND claimed_connector_id IS NULL`).bind(connector,row.id),
    connectorMutation,
    env.DB.prepare(`UPDATE connector_credentials SET revoked_at=CURRENT_TIMESTAMP WHERE connector_id=? AND revoked_at IS NULL`).bind(connector),
    ...
    env.DB.prepare(`UPDATE companies SET connector_status='connected',last_seen_at=CURRENT_TIMESTAMP WHERE id=? AND organization_id=? AND EXISTS(SELECT 1 FROM pairing_codes WHERE id=? AND claimed_connector_id=?)`).bind(row.company_id,row.organization_id,row.id,connector),
    env.DB.prepare(`UPDATE provisioning SET state='connector_connected',progress=10,error_code=NULL,error_message=NULL,updated_at=CURRENT_TIMESTAMP WHERE company_id=? AND organization_id=? AND state='awaiting_connector' AND EXISTS(SELECT 1 FROM pairing_codes WHERE id=? AND claimed_connector_id=?)`).bind(row.company_id,row.organization_id,row.id,connector),
  ...
  await env.DB.batch([
    env.DB.prepare(`
      ...
      WHERE id=? AND revoked_at IS NULL
        AND (last_seen_at IS NULL OR datetime(last_seen_at) IS NULL OR datetime(last_seen_at)<=datetime('now','-60 seconds')
    ...
    env.DB.prepare(`
      ...
      WHERE id=? AND organization_id=?
        AND (last_seen_at IS NULL OR datetime(last_seen_at) IS NULL OR datetime(last_seen_at)<=datetime('now','-60 seconds') OR connector_status IS NOT 'connected')
